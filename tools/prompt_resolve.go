...
// GetResolvedPrompt 返回当前配置选中的系统指令与版本名。
// 未配置 prompt 或 activeVersion 为空时，版本名为 "builtin"，内容同代码默认模板。
func GetResolvedPrompt() (system string, version string, err error) {
	_ = loadStockConfig()
	cfg := stockConfig
	if cfg == nil || cfg.Prompt == nil || strings.TrimSpace(cfg.Prompt.ActiveVersion) == "" {
		return prompt.DefaultSystemInstructionTemplate, "builtin", nil
	}
